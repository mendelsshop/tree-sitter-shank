...
bool tree_sitter_shank_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
...
unsigned tree_sitter_shank_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_shank_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
...
void *tree_sitter_shank_external_scanner_create() {
    ...
    tree_sitter_shank_external_scanner_deserialize(scanner, NULL, 0);
    return scanner;
}

void tree_sitter_shank_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
    array_delete(&scanner->indents);
    array_delete(&scanner->delimiters);
    free(scanner);
